                ...
                async with session.get(url=url, ssl=False) as response:
                    ...
                    if response.get("results"):
                        params = [
                            (
                                stock_id,
                                datetime.datetime.fromtimestamp(bar["t"] / 1000.0),
                                round(bar["o"], 2),
                                round(bar["h"], 2),
                                round(bar["l"], 2),
                                round(bar["c"], 2),
...
async def get_prices(pool, symbol_urls):
    try:
        ...
        ret = await asyncio.gather(
            *[
                get_price(pool, stock_id, symbol_urls[stock_id])
...
async def get_stocks():
    ...
    async with pool.acquire() as connection:
        stocks = await connection.fetch(
            "SELECT * FROM stock WHERE id IN (SELECT holding_id FROM etf_holding)"
        )
        ...
        for stock in stocks:
            symbol_urls[
                stock["id"]
            ] = f"https://api.polygon.io/v2/aggs/ticker/{stock['symbol']}/range/5/minute/2020-10-01/2021-02-05?apiKey={config.POLYGON_API_KEY}&limit=50000"
